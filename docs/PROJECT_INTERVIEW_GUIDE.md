...
> 我会先扩大冻结 holdout、持续运行 resolver 可达性预检，并强制对照实验的 prompt、Schema 和请求集合哈希一致。现有候选仍未达到工具 recall、澄清和边界拦截门槛，所以不会继续投入多 seed 稳定性训练。系统工程侧再根据实际部署需求，把 SQLite 迁移到 PostgreSQL 和队列 worker；模型或 prompt 更新必须先做轨迹 replay diff，再过 v1 无退化与系统安全双门槛。

### Q24：v2.3 解决了什么，为什么仍没有上线？

建议回答：

> v2.3 把 1716 条训练样本统一成单个 AgentAction 协议，移除了裸意图监督，并保证 2048 token 下零截断。后来我发现旧轨迹分数混入了 resolver、终态映射和槽位评分缺陷，所以没有继续用训练修系统 bug。我先把确定性可达上限修到 1.0，再在完全相同且有哈希记录的 AgentAction/XGrammar 契约下重评既有模型。control 为 15/18，v2.3 为 16/18，但 v2.3 的工具 recall 只有 0.8333、澄清准确率 0.9444、边界拦截 recall 0.50，仍低于门槛。intent Macro F1 的历史 0.8294 也没有稳定复现，本轮是 0.7991。因此我停止训练并保持 v1 为默认；支持的结论只是 v2.3 对该输出协议有方向性适配，不是它已经稳定或具备通用 Agent 能力。

...
> 我做的是一个面向多车型智能仓储的调度 Agent。当前演示只部署一个本地 Qwen2.5-1.5B QLoRA v2.3 服务，由有界 observe-decide-act loop 驱动；模型每轮只选择一个只读工具、请求澄清或提出结构化意图，工具结果和 verifier issues 会进入下一轮决策。服务端负责实体绑定、工具白名单、预算、注入隔离和 MASP 校验，大模型没有权限生成路径、写资源预约或控制车辆。可修复校验问题最多回送两次，权限、审批和过期状态直接阻断。linear v1 只保留代码兼容和历史评测，不再启动第二套演示服务。

更短的版本：

> 这是一个“LLM 负责理解，确定性引擎负责决策和执行”的安全闭环调度 Agent。本地 v2.3 模型把自然语言转换为受约束的调度意图，MASP 负责所有路径、资源和冲突计算，R3 高风险操作必须仿真后人工审批。
...
- 当前单模型演示使用统一单动作协议的 QLoRA v2.3；历史门禁没有证明它稳定优于 control，因此演示选型不能表述为生产晋级。
...
当前唯一运行路径是有真实回边的 `loop`；`linear` 只用于历史回归：
...
XGrammar 解决的是结构合法性，不是业务正确性。v2.3 修复后重评的 JSON 合法率达到 100%，但原始 Schema 有效率是 92%，而闭环目标成功率只有 88.9%。原因包括模型选择了 Schema 合法但任务语义不对的动作，或没有调用 gold 要求的证据工具。因此受约束解码不能替代工具轨迹 gold、权威实体检查和 MASP verifier。
...
> 它不是一次请求一次回复，也不是先规划完再顺序执行的流水线。loop 模式下模型每轮只能做一个动作，看到工具结果或 verifier issue 后再决定下一步；非法调用会成为 observation，fixable issue 可以有界修复。终局仍由确定性 verifier 决定，运行还有独立的决策、工具、修复、Token、成本、延迟和步数预算。当前只启动 v2.3，linear v1 仅保留代码兼容和历史报告。
...
> 我已经把轨迹回归从 18 条扩展到 100 条、10 个业务分层，并把 suite、prompt、Schema 和请求集合 hash 纳入门禁。下一步不会继续堆 seed 或训练版本，而是先针对百例结果暴露的软歧义、检索语境下的意图保持和工具 recall 做数据与协议设计；任何新数据都必须另建训练前冻结 holdout，不能继续使用已经看过的 v3 suite 证明泛化。系统工程侧再根据实际部署需求，把 SQLite 迁移到 PostgreSQL 和队列 worker。

### Q24：为什么单模型演示选择 v2.3，但评测仍写 `KEEP_V1`？

建议回答：

> v2.3 是唯一真正学习了 `CALL_TOOL`、`REQUEST_CLARIFICATION`、`PROPOSE_INTENT` 单动作协议的本地模型，因此作品演示选择它才能展示闭环 Agent；v1 只会解析裸意图。模型选择门禁回答的是“能否证明 v2.3 稳定优于 control 并用于生产晋级”，答案仍是否。百例回归中 control 为 76/100，v2.3 为 81/100，paired 差异 +5%，但 95% 区间 `[-2.03%, 12.03%]` 包含 0，工具 recall 也只有 0.885。删除双模型演示只是工程收敛，不篡改评测结论。

### Q25：为什么 100 条还是不能证明模型稳定？

建议回答：

> 扩到 100 条首先解决的是计分粒度：一条失败只影响 1%，`94%` 门槛就是明确的 94/100。但 10 个分层内的句子是相关改写，不是 100 次独立随机试验，而且 suite 是在模型训练和旧缺陷分析后编写的 post-training regression，不是盲测。它能更可靠地定位回归和业务短板，却不能替代新的训练前 holdout、多 seed 或真实分布数据。当前 paired 置信区间跨 0，所以我没有把 +5% 写成稳定增益。

...
- 做过统一单动作协议的 QLoRA v2.3 实验；它对最终 AgentAction/XGrammar 契约有单次方向性增益，但没有稳定复现且闭环门槛未通过，因此没有替换 v1。
...
> v2.3 把 1716 条训练样本统一成单个 AgentAction 协议，移除了裸意图监督，并保证 2048 token 下零截断。后来我发现旧轨迹分数混入了 resolver、终态映射和槽位评分缺陷，所以没有继续用训练修系统 bug。我先把确定性可达上限修到 1.0，再在完全相同且有 hash 记录的 AgentAction/XGrammar 契约下重评既有模型。为了避免 18 条用例中单例影响 5.56%，我又冻结了 100 条、10 分层回归；control 为 76/100，v2.3 为 81/100。paired 差异是 +5%，但 95% 区间 `[-2.03%, 12.03%]` 包含 0，且工具 recall 只有 0.885、软歧义只通过 2/10、边界拦截 recall 为 0.50。因此保持 v1 为默认；支持的结论只是方向性改善，不是稳定优于 control 或已经可上线。
